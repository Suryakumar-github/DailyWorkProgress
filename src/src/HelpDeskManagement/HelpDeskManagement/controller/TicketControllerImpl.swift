...
class TicketControllerImpl: TicketController {
    ...
    func createTicket(title ticketTitle: String, description ticketDescription: String, createdDate: Date, status: TicketStatus, userId: Int, issueType : IssueType)   throws
    {
        ...
        guard let delegater = delegate else {
            print("Delegate Controller is Nil. Cannot Procede Furthere")
            return
        }
        if   (try delegater.assignTicketToAgent(ticket: ticket)) {
    ...
    func updateTicketStatus(agent: Agent, ticketId: Int, status: TicketStatus)   throws -> Bool {
        ...
        if ticket.statusProperty == status {
            print("The tickets current status is already you choosed one : \(status)")
            return false
        }
    ...
    func assignUnassignedTickets()  throws {
        let result = try   ticketDao.getUnassignedTickets()
        switch result {
        case .success(let tickets) :
            for ticket in tickets {
                guard let delegater = delegate else {
                    print("Delegate function is Nil cannot procede further..")
                    return
                }
                if   (try delegater.assignTicketToAgent(ticket: ticket)) {
                      ticketDao.updateTicketStatus(ticketId: ticket.getTicketId, status: TicketStatus.assigned)
    ...
    func closeTicket(agent: Agent, ticketId: Int, solution : String)  throws -> Bool {
        ...
        if (ticket.statusProperty == TicketStatus.cancelled || ticket.statusProperty == TicketStatus.closed) {
            print("Ticket is Already Closed or Cancelled, can't Close Again..")
            return false
        }
